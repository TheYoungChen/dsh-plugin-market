window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		async function resolveRegistryRepo() {
			if (registryRepo !== null) return registryRepo;
			try {
				const payload = await (await fetch("/api/plugin-market/self")).json();
		...
		*/
		...
		function writeStoredRegistry(page) {
			try {
				window.localStorage.setItem(REGISTRY_STORAGE_KEY, JSON.stringify({
					items: page.items,
		...
		async function fetchRegistry() {
			...
			const urls = [`https://cdn.jsdelivr.net/gh/${repo}@main/registry.json`, `https://raw.githubusercontent.com/${repo}/main/registry.json`];
			...
			for (const url of urls) try {
				...
				if (!Array.isArray(payload.plugins)) {
					lastError = /* @__PURE__ */ new Error("registry response has no plugins list");
					continue;
				}
				const items = payload.plugins.map(toPlugin).filter((item) => item.fullName.length > 0 && !EXCLUDED.has(item.fullName));
				if (items.length === 0) {
					lastError = /* @__PURE__ */ new Error("registry response is empty");
					continue;
				}
				registryCache = {
					items,
		...
		*/
		async function fetchMarketPage(page, perPage, search = "", category = "all", sort = "stars") {
			try {
				const registry = await fetchRegistry();
				const counts = { all: registry.items.length };
				for (const item of registry.items) {
					const key = item.type ?? "other";
					counts[key] = (counts[key] ?? 0) + 1;
				}
				const term = search.trim().toLowerCase();
				const sorted = [...registry.items.filter((item) => {
		...
		async function fetchLatestVersion(fullName) {
			...
			try {
				...
				latestVersionCache.set(fullName, {
					at: Date.now(),
					version: pkg.version
				});
				return pkg.version;
		...
		*/
		async function fetchInstalled(force = false) {
			if (!force && installedCache !== null && Date.now() - installedCache.at < INSTALLED_TTL) return installedCache.value;
		...
		async function cleanupInstall(name) {
			...
			const payload = await response.json();
			if (!response.ok || payload.ok !== true) throw new Error(payload.message ?? `cleanup failed: ${response.status}`);
		}
		/** Uninstall a plugin: profile dependency/bundle + on-disk files. */
		async function uninstallInstall(name, type, repoName) {
			...
			const payload = await response.json();
			if (!response.ok || payload.ok !== true) throw new Error(payload.message ?? `uninstall failed: ${response.status}`);
		}
		//#endregion
		...
		*/
		...
		function MarketBrowser({ t }) {
			...
			const [uninstalling, setUninstalling] = (0, react.useState)(null);
			...
			(0, react.useEffect)(() => {
				fetchInstalled().then((report) => setInstalled(report.plugins), () => {});
			}, []);
			(0, react.useEffect)(() => {
				for (const job of jobs) if (job.status === "done" && !refreshedDone.current.has(job.id)) {
					refreshedDone.current.add(job.id);
					fetchInstalled(true).then((report) => setInstalled(report.plugins), () => {});
				}
			}, [jobs]);
			(0, react.useEffect)(() => {
			...
			const load = (page, search) => {
				...
				fetchMarketPage(page, PER_PAGE, search, category, sort).then((result) => setView({
					status: "ready",
			...
			const cleanupInstallEntry = async (info) => {
				try {
					await cleanupInstall(info.name);
					pushNotice(t("cleanup.notice", { name: info.name }));
				} catch {}
				try {
					setInstalled((await fetchInstalled(true)).plugins);
				} catch {}
			};
			...
			const counts = view.status === "ready" ? view.counts : void 0;
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						CATEGORIES.map((cat) => {
							...
							const label = cat === "all" ? t("category.all") : typeLabel(cat, t) ?? t("type.other");
							return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
								type: "button",
								style: {
									...chipStyle,
									...category === cat ? chipActiveStyle : {}
								},
								onClick: () => {
									setCategory(cat);
								},
								children: [label, /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									style: chipCountStyle,
									children: count
								})]
							}, cat);
						}),
							...
							children: sortedPlugins.map((plugin) => {
								...
								return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										...
										children: [
											info !== void 0 ? info.broken ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												style: {
													...installedTagStyle,
													color: "var(--dsw-alias-state-error-primary)"
												},
											...
											info !== void 0 ? info.broken ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
												type: "button",
												style: dangerButtonStyle,
												onClick: () => {
													cleanupInstallEntry(info);
												},
												children: t("cleanup")
											}) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [isUpdate(plugin) ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
												type: "button",
				...
				uninstalling !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					style: dialogBackdropStyle,
					onClick: () => {
						setUninstalling(null);
					},
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						role: "dialog",
						"aria-modal": "true",
						"aria-label": t("uninstall.confirm.title", { name: uninstalling.name }),
						onClick: stop,
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", {
								style: dialogTitleStyle,
								children: t("uninstall.confirm.title", { name: uninstalling.name })
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								style: dialogBodyStyle,
								children: t("uninstall.confirm.body", { name: uninstalling.name })
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								style: actionsStyle,
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									...
									children: t("uninstall.confirm.start")
								})]
							})
						]
					})
		...
		function InstallToast({ job, t }) {
			...
			if (isNotice) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				style: toastStyle,
				role: "status",
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		//#endregion
		//#region src/client/InstalledLayers.tsx
		...
		function LayerSection({ title, count, open, onToggle, children }) {
		...
		function InstalledLayers({ t, list }) {
			const [plugins, setPlugins] = (0, react.useState)([]);
			const [inventory, setInventory] = (0, react.useState)(null);
			const [userOpen, setUserOpen] = (0, react.useState)(true);
			const [systemOpen, setSystemOpen] = (0, react.useState)(false);
			const [confirmingUninstall, setConfirmingUninstall] = (0, react.useState)(null);
			const refresh = (force) => {
				fetchInstalled(force).then((report) => setPlugins(report.plugins), () => {});
				if (list !== void 0) list().then(setInventory, () => {});
			};
			(0, react.useEffect)(() => {
				refresh(false);
			}, [list]);
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)(LayerSection, {
				...
				children: [
					userPlugins.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						style: emptyStyle,
						children: t("manager.empty")
					}) : null,
					userPlugins.map((plugin) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						style: rowStyle,
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: rowNameStyle,
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: rowMetaStyle,
								children: plugin.version !== "" ? `v${plugin.version}` : ""
							}),
							confirmingUninstall?.name === plugin.name ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								...
								style: dangerActionStyle,
								onClick: () => {
									setConfirmingUninstall(plugin);
								},
								children: t("uninstall")
							})
						]
					}, plugin.name)),
					brokenPlugins.length > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
						style: {
							...emptyStyle,
							marginTop: 4
						},
						children: [
							t("manager.brokenSection"),
							" (",
							brokenPlugins.length,
							")"
						]
					}), brokenPlugins.map((plugin) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						style: rowStyle,
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: {
			...
			}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(LayerSection, {
				...
				children: [systemEntries.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
					style: emptyStyle,
					children: t("manager.emptySystem")
				}) : null, systemEntries.map((entry) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					style: rowStyle,
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							style: rowMetaStyle,
							children: entry.enabled ? t("manager.builtin") : t("manager.disabledSystem")
						})
					]
				}, entry.entryId))]
			})] });
		...
		function MarketSettingsTab({ t, list }) {
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				style: {
					display: "flex",
					flexDirection: "column",
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(InstalledLayers, {
						t,
						list
					}),
		...
		const zh = {
			...
			"cleanup": "清理",
			"uninstall": "卸载",
			"uninstall.confirm.title": "卸载 {name}",
			"uninstall.confirm.body": "将从 profile 移除 {name} 及其安装文件，重启后生效。此操作不可撤销。",
			"uninstall.confirm.start": "确认卸载",
			"uninstall.notice": "已卸载 {name}",
			"cleanup.notice": "已清理 {name}",
			"manager.title": "插件管理",
			"manager.userSection": "用户安装",
			"manager.brokenSection": "失效安装",
			"manager.systemSection": "系统自带",
			"manager.empty": "尚未安装任何插件",
			"manager.emptySystem": "无",
			"manager.builtin": "系统",
			"manager.disabledSystem": "未启用",
		...
		const en = {
			...
			"cleanup": "Clean up",
			"uninstall": "Uninstall",
			"uninstall.confirm.title": "Uninstall {name}",
			"uninstall.confirm.body": "This removes {name} and its files from the profile; it takes effect after restarting dsh. This cannot be undone.",
			"uninstall.confirm.start": "Uninstall",
			"uninstall.notice": "Uninstalled {name}",
			"cleanup.notice": "Cleaned up {name}",
			"manager.title": "Installed",
			"manager.userSection": "User-installed",
			"manager.brokenSection": "Broken",
			"manager.systemSection": "Built-in",
			"manager.empty": "No user plugins installed",
			"manager.emptySystem": "None",
			"manager.builtin": "System",
			"manager.disabledSystem": "Disabled",
